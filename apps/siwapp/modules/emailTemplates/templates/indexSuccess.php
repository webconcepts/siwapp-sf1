...
<div id="settings-wrapper" class="content">
  ...
  <form method="post" action="<?php echo url_for('@emailtemplates?action=save') ?>">
    ...
    <table id="listing" class="listing">
      ...
      <tbody>
        <?php foreach ($templates as $i => $template): ?>
          <tr id ="<?php echo "email-template-{$template->getId()}" ?>" class="template <?php echo "email-template-{$template->getId()}" ?>">
            <td class="check">
